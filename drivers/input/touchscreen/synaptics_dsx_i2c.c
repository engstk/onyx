...
static struct wake_lock tp_wake_lock;//????????,??2S,??suspend??,??resume??
...
static ssize_t synaptics_rmi4_baseline_data(char *buf, bool savefile)
{
	...
	//?????,????????3?WORD??,??????1000?, Limit ???:(-1,0.45),(-1,0.45),(-0.5,0.02)
	for(i = 0;i < 3; i++)
	{
		int iTemp[2];
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if(rmi4_data->gesture ) {
		...
		if(gesture[0]==0x0c) //mingqiang.guo@phone.bsp ?????????????0x0c,OPPO???,?????????????,
							 //????????,???????????,???????????
			gesture[0]=0x03;

		if(gesture[0] != 0) //???????????,???wake_lock,???????suspend??,????resume????,??suspend
...
static void __refdata synaptics_oneplus_func(struct work_struct *work)
{
	int retval;
